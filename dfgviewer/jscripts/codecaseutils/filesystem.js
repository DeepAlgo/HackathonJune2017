...
function pullFiles(){
    // copy the file pool
    var cpyFilePool = [].concat(filePool);
    filePool = [];
...
function loadFile(filename,callback,errcallback,cbowner)
{
...
//    $jit.id('inner-details').innerHTML = "loading " + filename;
	...
	{
		callback.apply(cbowner, [data].concat(args.slice(4,args.length)));
	}
	popFile(fileref);
	return;
    }
