...
public class Table {
        ...
        for (Pot pot : pots) {
            if (!pot.hasContributer(actor)) {
                ...
                if (newAmount >= potBet) {
                    ...
                    newAmount -= pot.getBet();
                } else {
                    // Partial call (all-in); redistribute pots.
                    pots.add(pot.split(actor, newAmount));
                    newAmount = 0;
                }
            }
            if (newAmount <= 0) {
    ...
    private void doShowdown() {
        ...
        if (lastBettor != null && !showingPlayers.contains(lastBettor)) {
    ...
    private void notifyMessage(String message, Object... args) {
        ...
        for (Player player : players) {
            player.getClient().messageReceived(newMessage);
        }
    }
